...
df = pd.read_excel('test3.xlsx', sheet_name=None)  # read all sheets
...
for key, value in Sheet2_Dict.items():
    # If artist is in artist list
    if value[0] in artists and value[1] in artists:
...
for key, value in albums_with_artists.items():
    ...
    for artist_value in artists_values:  # Iterating through all artists of an album
        ...
        if temp_artist in artists:  # Check if artist is in artist list or ont
            insert_df.loc[index_dict[temp_artist] - 1, value[0]] = -1
            insert_df.loc[index_dict[temp_artist] - 1, value[1]] = -1

    # print("Artists: ", artists_values)
    # artist_1 = artists_values[0]
    # artist_2 = artists_values[1]
    # # Updating values of first artist
    # print("Value: ", value)
    # print(index_dict[artist_1]-1, value[0])
